...
export const makeServerErrorHandler =
  ({ logMessage, message = "Lỗi hệ thống", status = 500, extra = {} }) =>
  (error, _req, res) =>
...
export const makeStatusMessageErrorHandler =
  ({ logMessage = null, fallbackMessage, extraKeys = [] }) =>
  (error, _req, res) => {
...
export const makePayloadErrorHandler =
  ({ logMessage = null, fallbackMessage = "Lỗi hệ thống" }) =>
  (error, _req, res) => {
...
export const makeJsonErrorHandler =
  ({ logMessage = null, status = 500, buildBody }) =>
  (error, _req, res) => {
    if (logMessage) {
...
export const makeValidationErrorHandler =
  ({ logMessage, serverMessage, validationMessage }) =>
  (error, _req, res) => {

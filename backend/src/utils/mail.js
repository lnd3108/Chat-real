...
export const sendVerificationCodeEmail = async ({ email, code, displayName }) => {
  const mailer = getTransporter();
  const appName = process.env.APP_NAME || "ChatRealTime";

  await mailer.sendMail({
    from: process.env.SMTP_FROM,
    to: email,
    subject: `${appName} - Mã xác minh đăng nhập`,
    text: [
    ...
    html: `
      <div style="font-family:Arial,sans-serif;line-height:1.6;color:#111827">
        ...
        <p>Mã xác minh đăng nhập của bạn là:</p>
...
const getMailConfigStatus = () => {
  ...
  if (
    ...
    return {
      ok: false,
      message: "SMTP vẫn đang dùng giá trị mẫu, chưa thay bằng tài khoản thật.",
    };
  }

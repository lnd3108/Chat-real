...
export const socketAuthMiddleWare = async (socket, next) => {
  try {
    ...
    if (accessUser.status === "banned") {
      return next(createSocketAuthError("Tài khoản đã bị khóa", "ACCOUNT_BANNED"));
    }
    ...
    if (!hasAdminPanelAccess(accessUser)) {
      ...
      if (maintenanceEnabled) {
        ...
        return next(createSocketAuthError(message, "MAINTENANCE_MODE"));
      }
    }

    socket.user = accessUser;
    return next();
  } catch (error) {
    ...
    return next(createSocketAuthError("Không thể xác thực socket", "TOKEN_INVALID"));
  }
};

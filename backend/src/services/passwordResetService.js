...
import { sendForgotPasswordOtpEmail } from "./mailService.js";
import {
...
const GENERIC_VERIFY_OTP_MESSAGE = "Mã xác nhận không hợp lệ, đã hết hạn hoặc đã được sử dụng.";
...
export const requestPasswordReset = async ({ email, req }) => {
  ...
  const refreshedLatestRequest = await getLatestPasswordResetRequest(normalizedEmail);
  const latestSentAt = refreshedLatestRequest?.lastSentAt?.getTime?.();

  if (latestSentAt && Date.now() < latestSentAt + PASSWORD_RESET_RESEND_COOLDOWN_MS) {
    throw {
      status: 429,
  ...
  const hourlyThreshold = new Date(
    Date.now() - 60 * 60 * 1000,
  );
...
export const verifyPasswordResetOtp = async ({ email, otp }) => {
  ...
  if (request.invalidatedAt || request.isUsed) {
    throw {
      status: 400,
      message: GENERIC_VERIFY_OTP_MESSAGE,
    };
...
export const resetPasswordWithVerifiedOtp = async ({
  ...
  const request = await PasswordResetOtp.findById(decoded.otpId);
  if (!request) {
    throw {
      status: 401,
      message: INVALID_RESET_TOKEN_MESSAGE,
